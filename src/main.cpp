...
{
 //TO DO - Get data from sensors and replace the 20
  ...
  (void)az_span_u32toa(temp_span, bme.temperature, &temp_span);  
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(","));
   temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(" \"humidity\": "));
  (void)az_span_u32toa(temp_span, bme.humidity,&temp_span);  
    temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(","));
   temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(" \"pressure\": "));
  (void)az_span_dtoa(temp_span, bme.pressure / 100.0,1 ,&temp_span);  
     temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(","));
   temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(" \"gas\": "));
  (void)az_span_dtoa(temp_span, bme.gas_resistance / 1000.0,1,&temp_span);  
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(","));
   temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(" \"altitude\": "));
  (void)az_span_dtoa(temp_span, bme.readAltitude(SEALEVELPRESSURE_HPA),1,&temp_span);
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(" }"));
...
static void sendTelemetry()
{
  ...
  //TODO change this from serial print out to getting values
 //STACEY COMMENTED OUT getSensorData();
...
void getSensorData(){
  ...
  if (! bme.performReading()) {
   ...
   Logger.Info("Gas = ");
   Logger.Info(bme.gas_resistance / 1000.0);
   Logger.Info(" KOhms");

   Logger.Info("Approx. Altitude = ");
...
// Stacey - put this checkin here to make sure we only send telemetry if we get a reading :) 
  if (! bme.performReading()) {
     Logger.Info("Failed to perform reading :(");
    return;
  }
